...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get goals
    const goalsPath = `/family/${currentMember.name}/finances/goals.json`
    const response = await fetch(
      `https://cloud-api.yandex.net/v1/disk/resources?path=${encodeURIComponent(goalsPath)}`,
      {
        headers: { Authorization: `OAuth ${token}` },
      },
    )
    ...
    if (response.ok) {
      const downloadResponse = await fetch(
        `https://cloud-api.yandex.net/v1/disk/resources/download?path=${encodeURIComponent(goalsPath)}`,
        {
          headers: { Authorization: `OAuth ${token}` },
        },
      )
      ...
      if (downloadResponse.ok) {
        const downloadData = await downloadResponse.json()
        const goalsResponse = await fetch(downloadData.href)
